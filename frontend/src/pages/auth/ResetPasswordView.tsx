...
import { Shield, Key, RefreshCw, AlertCircle, CheckCircle } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setSuccessMsg(data.message || 'Contraseña restablecida correctamente.');
      } else {
        setErrorMsg(data.error || 'El enlace de recuperación es inválido o ha expirado.');
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden select-none">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md z-10 text-center">
        ...
        <p className="mt-2 text-center text-sm text-slate-400">
          Ingrese su nueva credencial de acceso para actualizar la cuenta
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10">
        <div className="bg-slate-900/60 border border-slate-800/80 backdrop-blur-xl py-8 px-4 shadow-2xl rounded-2xl sm:px-10">
          ...
          {errorMsg && (
            <div className="mb-4 p-3.5 bg-rose-950/40 border border-rose-900/30 text-rose-450 text-xs rounded-xl flex items-start gap-2.5">
          ...
          {successMsg ? (
            <div className="space-y-4 text-center">
              <div className="mb-4 p-3.5 bg-emerald-950/40 border border-emerald-900/30 text-emerald-450 text-xs rounded-xl flex items-start gap-2.5 text-left">
                <CheckCircle size={16} className="flex-shrink-0 mt-0.5" />
                <span>{successMsg}</span>
              </div>
              <Link
                to="/login"
                className="mt-4 block w-full text-center py-2.5 px-4 border border-transparent rounded-xl shadow-md text-sm font-bold text-white bg-indigo-650 hover:bg-indigo-500 transition-all"
              >
