...
router.get('/tenants', adminCors, authenticateToken, requireSuperAdmin, async (req, res) => {
    const db = getDb();
    try {
...
router.get('/leads', adminCors, authenticateToken, requireSuperAdmin, async (req, res) => {
    const db = getDb();
    try {
...
router.put('/leads/:id', adminCors, authenticateToken, requireSuperAdmin, async (req, res) => {
    const db = getDb();
    const { id } = req.params;
...
router.put('/tenants/:id', adminCors, authenticateToken, requireSuperAdmin, async (req, res) => {
    const db = getDb();
    const { id } = req.params;
    const { sales_status, sales_notes } = req.body;
    try {
        const result = await db.query(`UPDATE users SET sales_status = $1, sales_notes = $2 WHERE id = $3 RETURNING *`, [sales_status, sales_notes, id]);
        if (result.rowCount === 0) {
