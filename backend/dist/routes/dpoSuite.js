...
router.get('/risks', adminCors, async (req, res) => {
    const db = getDb();
    try {
        const result = await db.query('SELECT * FROM risk_matrix WHERE user_id = $1 ORDER BY created_at DESC', [req.user.id]);
...
router.post('/risks', adminCors, async (req, res) => {
    ...
    const db = getDb();
    try {
        const result = await db.query(`INSERT INTO risk_matrix (user_id, process_name, identified_risk, severity, mitigation_control, status)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`, [
            req.user.id,
            process_name,
...
router.put('/risks/:id', adminCors, async (req, res) => {
    const { id } = req.params;
    ...
    try {
       ...
       RETURNING *`, [status, mitigation_control, id, req.user.id]);
...
router.delete('/risks/:id', adminCors, async (req, res) => {
    const { id } = req.params;
    const db = getDb();
    try {
        const result = await db.query('DELETE FROM risk_matrix WHERE id = $1 AND user_id = $2', [id, req.user.id]);
        if (result.rowCount === 0) {
...
router.get('/whistleblower', adminCors, async (req, res) => {
    const db = getDb();
    try {
        const result = await db.query('SELECT * FROM whistleblower_reports WHERE user_id = $1 ORDER BY reported_date DESC', [req.user.id]);
        res.json(result.rows);
    }
    catch (error) {
...
router.post('/whistleblower', adminCors, async (req, res) => {
    ...
    const db = getDb();
    try {
        const result = await db.query(`INSERT INTO whistleblower_reports (user_id, incident_description, status)
       VALUES ($1, $2, $3)
       RETURNING *`, [req.user.id, incident_description, status || 'PENDING']);
        res.status(201).json(result.rows[0]);
    }
    catch (error) {
...
router.patch('/whistleblower/:id/status', adminCors, async (req, res) => {
    const { id } = req.params;
    ...
    try {
       ...
       RETURNING *`, [status, id, req.user.id]);
        if (result.rowCount === 0) {
