...
router.post('/register', authRateLimiter, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const insertRes = await client.query('INSERT INTO users (email, password_hash, company_name, role) VALUES ($1, $2, $3, $4) RETURNING id, email, company_name, role', [email, passwordHash, companyName, 'tenant']);
        ...
        // Generate JWT
        const token = jwt.sign({ id: user.id, email: user.email, company_name: user.company_name, role: user.role, sv: deriveSessionVersion(user.id, passwordHash, JWT_SECRET) }, JWT_SECRET, { expiresIn: '8h', algorithm: 'HS256', issuer: 'scanner-dpo', audience: 'scanner-dpo-api' });
        res.status(201).json({
            success: true,
            token,
            user: {
