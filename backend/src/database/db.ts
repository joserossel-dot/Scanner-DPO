...
class MockPool {
  ...
  async query(queryText: string, params: any[] = []) {
    ...
    if (text.includes('FROM risk_matrix') && text.includes('user_id = $1')) {
      const list = store.riskMatrix?.filter((r: any) => r.user_id === params[0]) || [];
      return { rows: list, rowCount: list.length };
    }

    if (text.startsWith('INSERT INTO risk_matrix')) {
      if (!store.riskMatrix) store.riskMatrix = [];
      const newRisk = {
    ...
    if (text.startsWith('UPDATE risk_matrix')) {
      ...
      const risk = store.riskMatrix?.find((r: any) => r.id === id && r.user_id === userId);
      if (risk) {
        risk.status = status;
    ...
    if (text.startsWith('DELETE FROM risk_matrix')) {
      ...
      store.riskMatrix = store.riskMatrix?.filter((r: any) => !(r.id === id && r.user_id === userId)) || [];
    ...
    if (text.includes('FROM whistleblower_reports') && text.includes('user_id = $1')) {
      const list = store.whistleblowerReports?.filter((w: any) => w.user_id === params[0]) || [];
      return { rows: list, rowCount: list.length };
    }

    if (text.startsWith('INSERT INTO whistleblower_reports')) {
      if (!store.whistleblowerReports) store.whistleblowerReports = [];
      const newReport = {
    ...
    if (text.startsWith('UPDATE whistleblower_reports')) {
      ...
      const report = store.whistleblowerReports?.find((w: any) => w.id === id && w.user_id === userId);
      if (report) {
    ...
    if (text.includes('FROM ropa_inventory') && text.includes('user_id = $1')) {
      const list = store.ropaInventory?.filter((r: any) => r.user_id === params[0]) || [];
      return { rows: list, rowCount: list.length };
    }

    if (text.startsWith('INSERT INTO ropa_inventory')) {
      if (!store.ropaInventory) store.ropaInventory = [];
      const newRopa = {
    ...
    if (text.startsWith('UPDATE ropa_inventory')) {
      ...
      const item = store.ropaInventory?.find((r: any) => r.id === id && r.user_id === userId);
      if (item) {
    ...
    if (text.startsWith('DELETE FROM ropa_inventory')) {
      ...
      store.ropaInventory = store.ropaInventory?.filter((r: any) => !(r.id === id && r.user_id === userId)) || [];
