...
router.post('/', requireOrganizationPermission('incidents.manage'), async (req: any, res) => {
  const {
    domain,
    incident_title,
    incident_date,
  ...
  const requires_titulars_notification = affected_data_categories.some((cat: string) => {
...
router.put('/:id', requireOrganizationPermission('incidents.manage'), async (req: any, res) => {
  const { id } = req.params;
...
router.post('/:id/generate-notice', requireOrganizationPermission('incidents.manage'), async (req: any, res) => {
  const { id } = req.params;
  const db = getDb();
...
router.post('/scan-vulnerabilities', requireOrganizationPermission('incidents.manage'), async (req: any, res) => {
  ...
  try {
    ...
    const targetVulnerabilities = scanResult.vulnerabilities.filter(
    ...
    for (const vul of targetVulnerabilities) {
      ...
      const dupCheck = await db.query(
        `SELECT id FROM security_incidents 
         WHERE domain = $1 AND incident_title = $2 AND status != 'REPORTED_AND_CLOSED' AND organization_id = $3`,
        [domain, `[ALERTA PREVENTIVA] ${vul.title}`, req.organization.id]
      );
...
router.post('/assess-risk', requireOrganizationPermission('compliance.read'), async (req: any, res) => {
